...
# 2. 컬럼명 정리 및 특수 파일 처리
# ------------------------------
def clean_column_names(df, filename=""):
    """
    컬럼명을 정리하고, 특수한 구조의 파일을 처리합니다.
    """
    new_columns = {}
    
    # 특수 처리: 평생교육기관 파일
    if "평생교육" in filename and "Unnamed: 1" in df.columns:
    ...
    # 특수 처리: 위도경도 파일
    if "위도경도" in filename and "do" in df.columns:
        # do 컬럼을 region으로 사용 (시/군 정보는 나중에 집계)
        df["region"] = df["do"].astype(str).str.strip()
        # city, docity 컬럼 이름 정리
        if "city" in df.columns:
            df = df.rename(columns={"city": "city_detail"})
        if "docity" in df.columns:
            df = df.rename(columns={"docity": "docity_detail"})
        return df
    ...
    for col in df.columns:
        ...
        # 지역 관련 (docity도 추가)
        region_keywords = ["행정구역", "행정", "시도", "지역", "광역지자체", "docity", "do", "region", "Çà"]
        ...
        elif "기초지자체" in col or "city" in col_lower:
...
def standardize_region_names(df):
    ...
    # 5. 권역 데이터 제거 (합계, 수도권, 비수도권 등) + NaN 제거
    df = df[df["region"].notna()]  # NaN 제거
    exclude_keywords = ["합계", "전국", "수도권", "비수도권", "권역", "nan"]
    df = df[~df["region"].str.lower().isin(exclude_keywords)]
...
# 4. 광역지자체 데이터 집계
# ------------------------------
def aggregate_by_region(df, filename=""):
    """
    세부 데이터를 광역 단위로 집계합니다.
    - 디지털배움터: 기초지자체 데이터 합계
    - 위도경도: 각 광역시도의 중심 좌표 계산
    """
    if "region" not in df.columns:
        return df
    
    # 위도경도 파일: 평균 좌표 계산
    if "위도경도" in filename and "latitude" in df.columns and "longitude" in df.columns:
        print(f"  ℹ️  시/군 단위 좌표 → 광역 단위 평균 좌표 계산")
...
def main():
    ...
    for filename in files:
        ...
        # 전처리
        df = clean_column_names(df, filename)  # filename 전달
        ...
        # 세부 데이터 집계 (파일명 전달)
        if "city" in df.columns or "위도경도" in filename:
            df = aggregate_by_region(df, filename)
